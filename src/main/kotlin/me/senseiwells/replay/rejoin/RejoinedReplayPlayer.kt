...
class RejoinedReplayPlayer private constructor(
    ...
    companion object {
        fun rejoin(player: ServerPlayer, recorder: ReplayRecorder) {
            ...
            place(rejoined, RejoinGamePacketListener(rejoined, connection), player) {
        ...
        fun place(
            ...
            listener.send(ClientboundCustomPayloadPacket(ClientboundCustomPayloadPacket.BRAND, PacketByteBufs.create().writeUtf(this.server.serverModName)))
            listener.send(ClientboundChangeDifficultyPacket(levelData.difficulty, levelData.isDifficultyLocked))
            listener.send(ClientboundPlayerAbilitiesPacket(this.abilities))
            listener.send(ClientboundSetCarriedItemPacket(this.inventory.selected))
            listener.send(ClientboundUpdateRecipesPacket(server.recipeManager.recipes))
            listener.send(ClientboundUpdateTagsPacket(TagNetworkSerialization.serializeTagsToNetwork(this.server.registries())))
